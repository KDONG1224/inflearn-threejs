...
const init = () => {
  ...
  const ears = new THREE.BoxBufferGeometry(10, 10, 12);
  const earMaterial = new THREE.MeshLambertMaterial({ color: 0xffff00 });
  const earMesh = new THREE.Mesh(ears, earMaterial);
  earMesh.position.set(-20, 17.5, 0);
  // 귀

  const _ears = new THREE.BoxBufferGeometry(10, 10, 12);
  const _earMaterial = new THREE.MeshLambertMaterial({ color: 0xffff00 });
  const _earMesh = new THREE.Mesh(_ears, _earMaterial);
  _earMesh.position.set(20, 17.5, 0);
  // 귀
  ...
  faceGroup.add(boxMesh, cone, sphere1, sphere2, earMesh, _earMesh);
  //얼굴 그룹에 전부 넣기
